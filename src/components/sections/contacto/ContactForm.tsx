...
const PALLET_OPTIONS = [
  ...
  'Pallet con Tirante',
  'Pallet Doble Faz',
  'Medidas Especiales / A medida',
  'Pallet de Cartón (CarPallet)',
...
export default function ContactForm() {
  ...
  function buildWhatsAppUrl(data: FormState): string {
    const text = [
      'Hola PalletsJJ! Vengo del formulario web.',
  ...
  if (success) {
    return (
      <div className="border border-accent-green/30 bg-brand-white p-8 flex flex-col items-center text-center gap-4">
        ...
        <p className="text-base font-medium text-brand-dark">¡Mensaje enviado!</p>
        <p className="text-sm text-brand-tan leading-relaxed max-w-xs">
          Te redirigimos a WhatsApp con tu consulta pre-cargada.
          Si no se abrió automáticamente, hacé click en el botón.
        </p>
        <a
          href={waUrl}
          target="_blank"
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
      ...
      <div>
        ...
        />
        {errors.empresa && (
          <p className="text-xs text-accent-warm mt-1">{errors.empresa}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.nombre && (
          <p className="text-xs text-accent-warm mt-1">{errors.nombre}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.email && (
          <p className="text-xs text-accent-warm mt-1">{errors.email}</p>
        )}
      </div>
      ...
      <div>
        ...
        />
        {errors.telefono && (
          <p className="text-xs text-accent-warm mt-1">{errors.telefono}</p>
        )}
      </div>
      ...
      <div>
        ...
        <select
          ...
          value={form.tipo}
          onChange={handleChange}
          className={fieldClass(!!errors.tipo) + ' cursor-pointer'}
        >
        ...
        {errors.tipo && (
          <p className="text-xs text-accent-warm mt-1">{errors.tipo}</p>
        )}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          value={form.mensaje}
          onChange={handleChange}
          placeholder="Ej: Necesito 500 pallets euro por mes para distribución de productos de consumo masivo. Entrega en Pilar, Buenos Aires."
          className={fieldClass(!!errors.mensaje) + ' resize-none'}
        />
        {errors.mensaje && (
          <p className="text-xs text-accent-warm mt-1">{errors.mensaje}</p>
        )}
      </div>
